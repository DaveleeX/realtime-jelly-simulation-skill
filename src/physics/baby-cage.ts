...
export type JellyModelName='default'|'wukong';

export async function loadBabyCage(model:JellyModelName='default') {
  const urls=model==='wukong'
    ?[new URL('../assets/model/wukong.bin',import.meta.url),new URL('../assets/model/wukong.json',import.meta.url)]
    :[new URL('../assets/model/jelly-baby.bin',import.meta.url),new URL('../assets/model/jelly-baby.json',import.meta.url)];
  const [binary,metadata]=await Promise.all([
    fetch(urls[0]),fetch(urls[1]),
  ]);
  if(!binary.ok||!metadata.ok)throw new Error(`Could not load the ${model} jelly mesh`);
