...
export async function loadEnvironment(renderer:THREE.WebGPURenderer,scene:THREE.Scene) {
  const source=await new RGBELoader().setDataType(THREE.HalfFloatType).loadAsync(new URL('../assets/warm-bar.hdr',import.meta.url).href);
  ...
  const lighting=measureWindow(original);
  const pmrem=new THREE.PMREMGenerator(renderer);
  const target=pmrem.fromEquirectangular(source);
  scene.environment=target.texture;scene.environmentIntensity=1.35;
  scene.background=source;scene.backgroundBlurriness=.12;scene.backgroundIntensity=.8;
  return {...lighting,irradiance:lighting.irradiance*1.55,dispose:()=>{target.dispose();source.dispose();pmrem.dispose();}};
}
