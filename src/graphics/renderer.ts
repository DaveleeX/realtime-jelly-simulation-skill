...
export async function createRenderer(fail:(e:unknown)=>void) {
  ...
  renderer.toneMapping=THREE.AgXToneMapping;renderer.toneMappingExposure=1.24;
...
export function resizeView(renderer:THREE.WebGPURenderer,camera:THREE.PerspectiveCamera,controls:OrbitControls) {
  ...
  const dpr=Math.min(window.devicePixelRatio,mobile?1.5:1.7,Math.sqrt((mobile?1_800_000:4_000_000)/(width*height)));
  ...
  // Keep the camera in the upper hemisphere independently of screen aspect ratio.
  controls.minPolarAngle=.08;controls.maxPolarAngle=Math.PI/2-.08;
